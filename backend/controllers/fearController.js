...
export const getFearHistory = async (req, res) => {
  try {
    const history = await Behavior.find({ userId: req.user._id }).sort({
      createdAt: 1,
    });
...
export const logBehavior = async (req, res) => {
  try {
    ...
    } else if (action === "MATCHED_TRADE_ACCEPTED") {
      scoreDelta = -3; // Accepting behavioral match shows great discipline (strong fear reduction!)
    ...
    } else if (action === "QUIZ_RESULT" || action === "MOOD_REPORT") {
      // Custom delta sent via body (quiz answers or mood self-report)
  ...
  try {
    ...
    const portfolios = {
      HIGH: {
        ...
        expectedReturn: "4 - 6%",
        allocation: [
          { asset: "Bonds/FDs", pct: 70 },
          { asset: "Large Cap Stocks", pct: 30 },
        ],
      ...
      MEDIUM: {
        ...
        expectedReturn: "8 - 12%",
        allocation: [
          { asset: "Large Cap Stocks", pct: 60 },
          { asset: "Mid Cap Stocks", pct: 20 },
          { asset: "Bonds", pct: 20 },
        ],
      ...
      LOW: {
        ...
        expectedReturn: "15%+",
        allocation: [
          { asset: "Small Cap Stocks", pct: 40 },
          { asset: "Mid Cap Stocks", pct: 40 },
          { asset: "Crypto/Alt", pct: 20 },
        ],
